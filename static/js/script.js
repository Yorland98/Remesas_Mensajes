    ...
    if (window.innerWidth > 620) {
        submenuItems.forEach(submenu => {
            submenu.addEventListener('mouseover', () => {
                ...
                // Ya NO cerramos otros submenús, solo abrimos el actual
                const submenuList = submenu.querySelector('.submenu-items');
                if (submenuList) submenuList.classList.add('active');
            });
            
            // ELIMINAMOS el mouseleave individual de cada submenu
            // para que no se cierren individualmente
        });
        
        if (mobileMenu) {
            // Solo cerramos TODOS los submenús cuando salimos del menú completo
